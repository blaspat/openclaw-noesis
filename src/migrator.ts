...
export async function importMarkdownFiles(
  ...
  const result: ImportResult = { indexed: 0, skipped: 0, errors: 0, agentId };
  ...
  if (fs.existsSync(memoryMd)) filesToScan.push(memoryMd);
  ...
  const allChunks: Array<{ chunk: string; content: string; sourcePath: string; memoryType: MemoryType }> = [];
