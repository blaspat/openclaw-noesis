...
import { autoConfigOllama, contentChecksum, chunkText } from "./ollama.js";
import { NoesisDB } from "./lancedb.js";
import { hybridSearch } from "./search.js";
import { importMarkdownFiles, importAllAgents } from "./migrator.js";
import { startQmdWatcher, startMemoryWatcher } from "./watcher.js";
...
export default definePluginEntry({
    id: "noesis",
    name: "Noesis Memory",
    description: "Local-first semantic memory for OpenClaw — LanceDB + Ollama, no cloud required. Hybrid search (vector + BM25 + MMR), cross-agent recall, QMD session indexing, and memory slot integration.",
    register(api) {
        ...
        api.registerTool({
            name: "noesis_index",
            label: "Noesis Index",
            description: "Store a memory entry in Noesis semantic memory. Embeds and indexes the content for future semantic search. Idempotent — duplicate content is skipped automatically.",
        ...
        api.registerTool({
            name: "noesis_search",
            label: "Noesis Search",
            description: "Semantic search across Noesis memory. Uses hybrid pipeline: vector ANN + BM25 keyword search, reranked with MMR for diversity.",
            parameters: Type.Object({
            ...
            async execute(_toolCallId, params) {
                ...
                if (results.length < 2) {
                    ...
                    const [vectorArchive, ftsArchive] = await Promise.all([
                        db.searchArchive(embedding, topK * 2, params.agentId ? String(params.agentId) : undefined, params.memoryType),
                        db.fullTextSearchArchive(query, topK, params.agentId ? String(params.agentId) : undefined, params.memoryType),
                    ]);
        ...
        api.registerTool({
            ...
            description: "Search archived (expired) memories. These are entries that reached their TTL and were moved to long-term archive. Use this for 'search older memories' queries.",
            parameters: Type.Object({
                query: Type.String({ description: "Search query for archived memories" }),
                agentId: Type.Optional(Type.String({ description: "Filter by agent ID" })),
                memoryType: Type.Optional(Type.Union([Type.Literal("fact"), Type.Literal("decision"), Type.Literal("preference"), Type.Literal("context"), Type.Literal("session")])),
            ...
            async execute(_toolCallId, params) {
                ...
                const [vectorResults, ftsResults] = await Promise.all([
                    db.searchArchive(embedding, topK * 2, params.agentId ? String(params.agentId) : undefined, params.memoryType),
                    db.fullTextSearchArchive(queryText, topK, params.agentId ? String(params.agentId) : undefined, params.memoryType),
                ]);
                const scoreMap = new Map();
        ...
        api.registerTool({
            name: "noesis_recall",
            label: "Noesis Recall",
            description: "Recall recent memory entries for an agent or session. Ordered by creation time, newest first.",
            parameters: Type.Object({
                agentId: Type.Optional(Type.String({ description: "Filter by agent ID" })),
            ...
            async execute(_toolCallId, params) {
                await ensureInitialized();
                const entries = await db.recall(params.agentId ? String(params.agentId) : undefined, params.sessionId ? String(params.sessionId) : undefined, params.limit ? Number(params.limit) : 20);
                if (entries.length === 0) {
                    return {
                ...
                const formatted = entries
                    .map((e) => `[${new Date(e.createdAt).toISOString()}] [${e.memoryType}] ${e.content.slice(0, 200)}${e.content.length > 200 ? "…" : ""}`)
                    .join("\n\n");
                return {
        ...
        api.registerTool({
            name: "noesis_import",
            label: "Noesis Import",
            description: "Trigger MD → LanceDB migration for one agent or all agents. Scans memory/*.md and MEMORY.md files, chunks, embeds, and upserts. Idempotent.",
            parameters: Type.Object({
                agentId: Type.Optional(Type.String({ description: "Agent to import (omit to import all agents)" })),
            }),
            async execute(_toolCallId, params) {
                await ensureInitialized();
                const logLines = [];
                ...
                if (params.agentId) {
                    const result = await importMarkdownFiles(String(params.agentId), db, ollama, resolvedConfig, log);
                    return {
                        content: [
                            {
                                type: "text",
        ...
        api.registerTool({
            ...
            async execute(_toolCallId, params) {
                ...
                if (!updated)
                    return { content: [{ type: "text", text: `Entry ${id} not found.` }], details: {} };
                return {
        ...
        api.registerTool({
            ...
            async execute(_toolCallId, params) {
                ...
                const content = String(params.content ?? "").trim();
                if (!content)
                    return { content: [{ type: "text", text: "No content." }], details: {} };
                const agentId = String(params.agentId ?? "unknown");
            ...
            api.registerContextEngine("noesis", () => {
                return {
                    ...
                    async assemble({ sessionId, messages }) {
                        ...
                        const memories = await db.queryByPriority(resolvedConfig.assembleInjectPriority, undefined, // cross-agent
                        resolvedConfig.assembleMaxEntries, resolvedConfig.assembleMaxAgeDays > 0 ? resolvedConfig.assembleMaxAgeDays : undefined);
...
// ─── initialization ────────────────────────────────────────────────────────
async function initPlugin(config, log) {
