 ...
 */
export function startQmdWatcher(db, ollama, config, logger) {
 ...
 */
export function startMemoryWatcher(db, ollama, config, logger) {
 ...
 */
function agentIdFromMemoryPath(filePath) {
...
function inferMemoryType(content) {
    ...
    if (lower.includes("decided") || lower.includes("decision") || lower.includes("chose to") || lower.includes("going with"))
        return "decision";
    if (lower.includes("prefer") || lower.includes("like") || lower.includes("always") || lower.includes("never"))
        return "preference";
    if (lower.includes("fact:") || lower.includes("fact-"))
        return "fact";
    if (lower.includes("session") || lower.includes("yesterday") || lower.includes("last"))
        return "context";
    return "fact";
}
/**
 ...
 */
async function indexMemoryFile(filePath, db, ollama, config, logger) {
 ...
 *   ~/.openclaw/agents/<agentId>/qmd/sessions/<sessionId>.jsonl
 */
function parseSessionPath(filePath) {
    const home = os.homedir();
 ...
 */
async function indexSessionFile(filePath, db, ollama, config, logger) {
    const raw = fs.readFileSync(filePath, "utf-8");
