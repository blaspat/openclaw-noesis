 ...
 */
export async function hybridSearch(query, ollama, db, config, options = {}) {
    ...
    const vectorResults = await db.vectorSearch(queryEmbedding, topK * 2, options.agentId, options.memoryType, options.crossAgent);
    // 3. BM25 full-text search
    const bm25Results = await db.fullTextSearch(query, topK, options.agentId, options.memoryType, options.crossAgent);
    ...
    const withPriority = merged.sort((a, b) => {
        const ap = a.result.priority ?? 0;
        const bp = b.result.priority ?? 0;
        if (ap >= 75 && bp < 75)
            return -1;
        if (bp >= 75 && ap < 75)
            return 1;
 ...
 */
function mmrRerank(candidates, queryEmbedding, topK, lambda) {
